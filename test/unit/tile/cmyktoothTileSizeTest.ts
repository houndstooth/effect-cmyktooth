...
describe('cmyktooth tile size', () => {
	it('reduces the size by the square root of two each layer', () => {
		expect(isCloseTo(from.Unit(cmyktoothTileSize.main(to.Unit(2))), SQRT_2)).toBe(true)
		expect(isCloseTo(from.Unit(cmyktoothTileSize.main(to.Unit(SQRT_2))), 1)).toBe(true)
	})
})
